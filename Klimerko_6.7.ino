 ...
 * ------------------------------------------------------------------------------------------------------------
 * Version 6.7 Pressure Altitude Fix: 
 * - CHANGED: 'altitude' asset now shows "Pressure Altitude" based on 1013.25 hPa standard.
 * (Example: If QNH is 1017.9hPa at 386m, this will show approx 344m).
 * - KEPT: 'pressureSea' is still calculated correctly based on your fixed input (386m).
 * - INCLUDES: Fan Check, Counts, HumComp, Stack Fix, Remote features.
 */
...
// Varijable za Fan Check
int prev_pm1 = -1; int prev_pm25 = -1; int prev_pm10 = -1;
int stuckCounter = 0; int zeroCounter = 0;       
String sensorStatus = "Init"; 

// Varijable za Particle Counts
int pCount_0_3 = 0; int pCount_0_5 = 0; int pCount_1_0 = 0;
int pCount_2_5 = 0; int pCount_5_0 = 0; int pCount_10_0 = 0;

String pendingUpdateUrl = "";
bool shouldStartConfig = false; 

String firmwareVersion = "6.7 PressAlt"; 
const char* firmwareVersionPortal = "<p>Firmware Version: 6.7 (Pressure Altitude Logic)</p>";
char klimerkoID[32];
...
uint16_t MQTT_MAX_MESSAGE_SIZE = 4096; 
...
// Asset Definitions
const char* PM1_ASSET = "pm1"; const char* PM2_5_ASSET = "pm2-5"; const char* PM10_ASSET = "pm10";
const char* PM1_CORR_ASSET = "pm1-c"; const char* PM2_5_CORR_ASSET = "pm2-5-c"; const char* PM10_CORR_ASSET = "pm10-c";
const char* COUNT_0_3_ASSET = "count-0-3"; const char* COUNT_0_5_ASSET = "count-0-5"; const char* COUNT_1_0_ASSET = "count-1-0";
const char* COUNT_2_5_ASSET = "count-2-5"; const char* COUNT_5_0_ASSET = "count-5-0"; const char* COUNT_10_0_ASSET = "count-10-0";
...
void publishSensorData() {
  // FIX: STATIC buffer da ne trosi Stack memoriju (sprecava crash)
  static char JSONmessageBuffer[2048];
  ...
  if (pmsSensorOnline) {
    ...
    doc.createNestedObject(COUNT_0_3_ASSET)["value"] = pCount_0_3;
    doc.createNestedObject(COUNT_0_5_ASSET)["value"] = pCount_0_5;
    doc.createNestedObject(COUNT_1_0_ASSET)["value"] = pCount_1_0;
    doc.createNestedObject(COUNT_2_5_ASSET)["value"] = pCount_2_5;
    doc.createNestedObject(COUNT_5_0_ASSET)["value"] = pCount_5_0;
    doc.createNestedObject(COUNT_10_0_ASSET)["value"] = pCount_10_0;

    if (avgPM1 == prev_pm1 && avgPM25 == prev_pm25 && avgPM10 == prev_pm10) stuckCounter++; else stuckCounter = 0;
    if (avgPM1 == 0 && avgPM25 == 0 && avgPM10 == 0) zeroCounter++; else zeroCounter = 0;
    prev_pm1 = avgPM1; prev_pm25 = avgPM25; prev_pm10 = avgPM10;
    if (stuckCounter >= 12) sensorStatus = "Fan Stuck / Error";
    else if (zeroCounter >= 12) sensorStatus = "Zero Data Error";
    else sensorStatus = "OK";
    doc.createNestedObject(SENSOR_STATUS_ASSET)["value"] = sensorStatus;

    int pm1_c = avgPM1; int pm25_c = avgPM25; int pm10_c = avgPM10;
    if (bmeSensorOnline && avgHumidity > 70.0) {
        float correctionFactor = 1.0 + 0.015 * (avgHumidity - 70.0);
        pm1_c = (int)(avgPM1 / correctionFactor);
        pm25_c = (int)(avgPM25 / correctionFactor);
        pm10_c = (int)(avgPM10 / correctionFactor);
    }
    doc.createNestedObject(PM1_CORR_ASSET)["value"] = pm1_c;
    doc.createNestedObject(PM2_5_CORR_ASSET)["value"] = pm25_c;
    doc.createNestedObject(PM10_CORR_ASSET)["value"] = pm10_c;

  } else {
    Serial.println(F("[DATA] PMS Offline"));
  ...
  if (strlen(JSONmessageBuffer) > (unsigned int)(MQTT_MAX_MESSAGE_SIZE - 50)) 
      Serial.println(F("[DATA] WARN: Payload big!"));
...
void readPMS() { 
  ...
  if (pms.readUntil(data)) {
    ...
    pCount_0_3 = data.PM_RAW_0_3;
    pCount_0_5 = data.PM_RAW_0_5;
    pCount_1_0 = data.PM_RAW_1_0;
    pCount_2_5 = data.PM_RAW_2_5;
    pCount_5_0 = data.PM_RAW_5_0;
    pCount_10_0 = data.PM_RAW_10_0;
...
void readBME() { 
  ...
  // 1. Racunanje PRITISKA MORA na osnovu UNETE visine (fiksno)
  ...
  if (temperatureRaw > -100 && temperatureRaw < 150 && humidity >= 0 && humidity <= 110) {
    ...
    // 2. IZMENA: Racunanje VISINE PRITISKA na osnovu standardnih 1013.25
    ...
    if (bmeSensorOnline) {
      Serial.println(F("[BME] Invalid Data"));
      bmeSensorRetry++;
...
void publishDiagnosticData() { 
  if (!wifiConnectionLost && !mqttConnectionLost) {
    // FIX: Smanjen buffer za dijagnostiku da ne pukne Stack
    char JSONmessageBuffer[768];
...
void factoryReset() { 
  ...
  EEPROM.begin(sizeof(Settings));
  // FIX: Unsigned int loop counter
  for (unsigned int i=0; i < sizeof(Settings); i++) EEPROM.write(i, 0);
  EEPROM.commit(); EEPROM.end();
  ESP.restart();
}

void wifiConfigWebServerStarted() { wm.server->on("/exit", wifiConfigStop); }
// FIX: Unused parameter warning fix
...
void mqttCallback(char* p_topic, byte* p_payload, unsigned int p_length) {
  ...
  if (p_length >= 1024) return;
  DynamicJsonDocument doc(1024); char json[1024];
